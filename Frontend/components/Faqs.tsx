...
const AccordianItem = ({
  ...
  return (
    ...
    id = "faqs"
      className={clsx(
        "py-5 border-b border-white/10 cursor-pointer transition-all duration-300",
        isOpen ? "hover:border-yellow-400/30" : "hover:border-pink-400/30"
      )}
      onClick={() => setIsOpen(!isOpen)}
    >
      <div className="flex items-center gap-4">
        <span className="flex-1 text-base sm:text-lg font-semibold text-white transition-colors">
          {question}
        </span>
        <Image
      ...
      <div
        className={clsx(
          "mt-3 text-gray-400 text-sm sm:text-base overflow-hidden transition-all duration-500",
          {
            "max-h-0": !isOpen,
            "max-h-96 pb-4": isOpen,
          }
      ...
      <div className="container mx-auto relative z-10 px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="mt-6 text-lg sm:text-xl max-w-3xl mx-auto text-gray-300">
            ...
            <span className="text-pink-600">Contact our support team</span>.
          </p>
        </div>

        <div className="max-w-4xl mx-auto bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8 backdrop-blur-md hover:shadow-xl transition duration-300">
          {items.map(({ question, answer }) => (
            <AccordianItem key={question} question={question} answer={answer} />
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <button
            className="relative bg-gradient-to-r from-purple-600 to-purple-400 text-white py-3 px-8 rounded-full font-semibold transition hover:shadow-purple-600/50 hover:scale-105"
            onClick={() =>
              (window.location.href = "http://codehurdle.com/auth/google")
            }
          >
            <span className="relative z-10">Get Started Now</span>
            <span className="absolute inset-0 bg-[linear-gradient(90deg,#FFDEE9,#B5FFFC)] opacity-0 group-hover:opacity-100 transition-opacity duration-500 mix-blend-overlay"></span>
          </button>
        </div>
      </div>
    </div>
